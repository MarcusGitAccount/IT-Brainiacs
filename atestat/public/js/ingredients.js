...
{
  return `<div class="col-sm-6 col-xs-6 col-md-4 col-lg-4 fixed-height">
...
buttonSubmit.addEventListener('click', (e) => {
  ...
  let imageName = inputs[3].value.match(/[\w\d.]+/g);
  let nbr = parseInt(document.querySelector('li:last-of-type>span').innerHTML);

  XHR.open('POST', '/api/ingredients', true);
  XHR.onload = function(oEvent) {
    if (XHR.status == 200)   
      addIngredient(inputs[0].value, inputs[1].value, inputs[2].value, inputs[3].value.match(/[\w\d.]+/g)[2]), 7500;
